...
describe "SocialProfile" do        
    ...
    describe "can create and save valid instances " do
        it "can instantiate with a name, handle, and platform" do
            ...
            # expect(profile.platform_id).to eq(platform.id)
        end
    end
    ...
    describe "has a required, unique name (case insensitive) and provides correct error message when invalid" do
        ...
        it "won't instantiate when name is same as other instance (case insensitive)" do
            ...
            # valid_instance.platform = platform            
            same_name = SocialProfile.new(test_attrs)
            # same_name.platform = platform
            same_name_cap = SocialProfile.new(test_attrs_cap)
            # same_name_cap.platform = platform
    ...
    describe "has a required, properly formatted handle that is unique to the associated platform (case insensitive) and provides correct error message when invalid" do
        it "won't instantiate without a handle" do
            ...
            expect(no_handle.errors.messages[:handle]).to include("You must provide your profile handle.")
        end
        ...
        it "won't instantiate when handle has punctuation (except dots, hyphen, underscores) or special characters (except @)" do
            ...
            # test_char.platform = platform
            test_punc = SocialProfile.new(test_punc_attrs)
            # test_punc.platform = platform

            expect(test_char.save).to be false
            expect(test_char.errors.messages[:handle]).to include("That handle doesn't look valid. Please provide another.")
            expect(test_punc.save).to be false
            expect(test_punc.errors.messages[:handle]).to include("That handle doesn't look valid. Please provide another.")
        end
    ...
    describe "works with assoicated models correctly" do
        it "belongs to social_platform" do
            ...
            profile1.platform = platform
            profile1.save
            profile2 = SocialProfile.new(profile2_attrs)
            profile2.platform_id = platform.id
            profile2.save

            expect(profile1.save).to be true
            expect(profile1.platform.name).to eq("platform name")
            expect(profile1.platform.base_url).to eq("www.example.com/")
            expect(profile1.platform.image_file_name).to eq("icon.png")
            expect(profile2.save).to be true
            expect(profile2.platform.name).to eq("platform name")
            expect(profile2.platform.base_url).to eq("www.example.com/")
            expect(profile2.platform.image_file_name).to eq("icon.png")
        end
        ...
        it "won't instantiate unless associated to a platform" do
            ...
            # valid_platform = SocialPlatform.create(valid_platform_attrs)
            invalid_platform = SocialPlatform.create(invalid_platform_attrs)

            no_platform_attrs = {name: "profile1 name", handle: "@profhandle1"}
            bad_platform_attrs = {name: "profile2 name", handle: "@profhandle2"}

            no_platform = SocialProfile.new(no_platform_attrs)
            bad_platform = SocialProfile.new(bad_platform_attrs)
            bad_platform.platform = invalid_platform            
            
            expect(no_platform.save).to be false
            expect(no_platform.errors.messages[:handle]).to include("Your profile must belong to a platform.")
            expect(bad_platform.save).to be false
            expect(bad_platform.errors.messages[:handle]).to include("Your profile must belong to a platform.")
        end
    end
    ...
    describe "all helper methods work correctly" do
        ...
        it "can take a user's handle entry and format it to set up correct link text for view" do
            ...
            # has_at_spaces.platform = platform
            has_at_spaces.save            
            has_dots = SocialProfile.new(has_dots_attrs)
            # has_dots.platform = platform
            has_dots.save            
